...
import {closeModalCreateProperty,updateProperty} from '../store/actions/booking'
...
class ModalCreateProperty extends React.Component {
  ...
  uploadFile = (file, metadata, namePicture) => {
    ...
    const uploadTask = firebase.storage().ref().child(filePath).put(file, metadata)
  ...
  updateProperties = (url) => {
    ...
    const { _id,index,price,picture,city,address ,latitude,longitude,bedrooms,bathrooms,carSpaces} = this.state
    const data = {_id,index,price: +price,picture,city,address,latitude: +latitude,longitude: +longitude,bedrooms,bathrooms,carSpaces}
  ...
  updateDataFirebase = (data) =>{
    firebase.database().ref("data/-M5nKpHmlxdjPIfwYVs8")
        .child('properties')
        .push()
        .set(data)
  }
...
export default connect(mapStateToProps,mapDispatchToProps)(ModalCreateProperty)